# これなに？
**MinManager** は鉱物標本のコレクション管理をするためのアプリケーションです  
デスクトップアプリケーションとして作成しているため、 OFFLINE で動作します  
(ONLINE機能以外では) 外部と通信しないので安心してお使いいただけます  

(WPFなのでWindows**のみ**対応です、**mac対応はできません**ので悪しからずご了承ください)
...
https://github.com/nokatsu/MinManager/releases/tag/v1.2.0
...
* 改変行為を禁止します
* 商用利用を禁止します
    * 有償無償を問わず本アプリケーション(および設定ファイル等)の販売行為を指します
	* 業務での利用を制限しません  
